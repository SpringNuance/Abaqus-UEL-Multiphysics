...
module common_block
    ...
    real(kind=dp) :: all_N_inpt_to_knode(8, 8) ! (nnode, ninpt)
    real(kind=dp) :: all_N_node_to_kinpt(8, 8) ! (ninpt, nnode)
...
subroutine calc_N_inpt_to_knode(xi_node, eta_node, zeta_node, temp_N_inpt_to_knode)
    ! Calculate the shape function at the nodal points
    ! xi_node, eta_node, zeta_node: Isoparametric coordinates of the nodal points

    use precision
    real(kind=dp), dimension(8) :: temp_N_inpt_to_knode ! ninpt
    real(kind=dp) :: xi_node, eta_node, zeta_node

    !   shape functions
    temp_N_inpt_to_knode(1) = 0.125d0 * (1.d0 - xi_node) * (1.d0 - eta_node) * (1.d0 - zeta_node)
    temp_N_inpt_to_knode(2) = 0.125d0 * (1.d0 + xi_node) * (1.d0 - eta_node) * (1.d0 - zeta_node)
    temp_N_inpt_to_knode(3) = 0.125d0 * (1.d0 - xi_node) * (1.d0 + eta_node) * (1.d0 - zeta_node)
    temp_N_inpt_to_knode(4) = 0.125d0 * (1.d0 + xi_node) * (1.d0 + eta_node) * (1.d0 - zeta_node)
    temp_N_inpt_to_knode(5) = 0.125d0 * (1.d0 - xi_node) * (1.d0 - eta_node) * (1.d0 + zeta_node)
    temp_N_inpt_to_knode(6) = 0.125d0 * (1.d0 + xi_node) * (1.d0 - eta_node) * (1.d0 + zeta_node)
    temp_N_inpt_to_knode(7) = 0.125d0 * (1.d0 - xi_node) * (1.d0 + eta_node) * (1.d0 + zeta_node)
    temp_N_inpt_to_knode(8) = 0.125d0 * (1.d0 + xi_node) * (1.d0 + eta_node) * (1.d0 + zeta_node)

return
end

subroutine calc_N_node_to_kinpt(xi_int, eta_int, zeta_int, temp_N_node_to_kinpt)
    ...
    use precision
    real(kind=dp), dimension(8) :: temp_N_node_to_kinpt ! nnode
    real(kind=dp) :: xi_int, eta_int, zeta_int

    !   shape functions
    temp_N_node_to_kinpt(1)=0.125d0 * (1.d0 - xi_int) * (1.d0 - eta_int) * (1.d0 - zeta_int)
    temp_N_node_to_kinpt(2)=0.125d0 * (1.d0 + xi_int) * (1.d0 - eta_int) * (1.d0 - zeta_int)
    temp_N_node_to_kinpt(3)=0.125d0 * (1.d0 + xi_int) * (1.d0 + eta_int) * (1.d0 - zeta_int)
    temp_N_node_to_kinpt(4)=0.125d0 * (1.d0 - xi_int) * (1.d0 + eta_int) * (1.d0 - zeta_int)
    temp_N_node_to_kinpt(5)=0.125d0 * (1.d0 - xi_int) * (1.d0 - eta_int) * (1.d0 + zeta_int)
    temp_N_node_to_kinpt(6)=0.125d0 * (1.d0 + xi_int) * (1.d0 - eta_int) * (1.d0 + zeta_int)
    temp_N_node_to_kinpt(7)=0.125d0 * (1.d0 + xi_int) * (1.d0 + eta_int) * (1.d0 + zeta_int)
    temp_N_node_to_kinpt(8)=0.125d0 * (1.d0 - xi_int) * (1.d0 + eta_int) * (1.d0 + zeta_int)
...
subroutine calc_B_deriv_local_kinpt(xi_int, eta_int, zeta_int, temp_B_deriv_local_kinpt)
    ...
    ! Basically derivatives of calc_N_node_to_kinpt
...
subroutine UEXTERNALDB(lop,lrestart,time,dtime,kstep,kinc)
    ...
    real(kind=dp), dimension(ninpt) :: temp_N_inpt_to_knode
    real(kind=dp), dimension(nnode) :: temp_N_node_to_kinpt
    ...
    if (lop == 0) then 
        ...
        do knode = 1, nnode
            ...
            call calc_N_inpt_to_knode(xi_node, eta_node, zeta_node, temp_N_inpt_to_knode)
            all_N_inpt_to_knode(knode, 1:ninpt) = temp_N_inpt_to_knode

        end do      

        !print *, "all_N_inpt_to_knode: ", all_N_inpt_to_knode
        ...
        !   determine (g,h,r)
            ...
            call calc_N_node_to_kinpt(xi_int, eta_int, zeta_int, temp_N_node_to_kinpt)
            all_N_node_to_kinpt(kinpt, 1:nnode) = temp_N_node_to_kinpt
...
subroutine kjacobian(jelem,ndim,nnode,coords,B_deriv_local_kinpt,djac,B_deriv_global_kinpt,mcrd)

!     Notation: djac - Jac determinant; xjaci - inverse of Jac matrix
!     B_deriv_global_kinpt - shape functions derivatives w.r.t. global coordinates for current kinpt
    use precision
    include 'aba_param.inc'
    real(kind=dp) :: djac,inv_djac
    real(kind=dp), dimension(ndim,ndim) :: xjac,xjaci
    real(kind=dp), dimension(mcrd,nnode) :: coords
    real(kind=dp), dimension(ndim,nnode) :: B_deriv_local_kinpt
        ...
        do kdim=1,ndim
            do jdim=1,ndim
                xjac(jdim,kdim)=xjac(jdim,kdim) + &
                    B_deriv_local_kinpt(jdim,knode) * coords(kdim,knode)
            end do
        end do
    end do
...
subroutine kbmatrix_full(B_deriv_global_kinpt,ntens,nnode,ndim,Bu_kinpt)
    ...
    do knode=1,nnode
        ...
        ! Normal components
        Bu_kinpt(1,current_node_idx+1) = B_deriv_global_kinpt(1,knode)
        Bu_kinpt(2,current_node_idx+2) = B_deriv_global_kinpt(2,knode)
        Bu_kinpt(3,current_node_idx+3) = B_deriv_global_kinpt(3,knode)
        ! Shear components
        Bu_kinpt(4,current_node_idx+1) = B_deriv_global_kinpt(2,knode)
        Bu_kinpt(4,current_node_idx+2) = B_deriv_global_kinpt(1,knode)
        Bu_kinpt(5,current_node_idx+1) = B_deriv_global_kinpt(3,knode)
        Bu_kinpt(5,current_node_idx+3) = B_deriv_global_kinpt(1,knode)
        Bu_kinpt(6,current_node_idx+2) = B_deriv_global_kinpt(3,knode)
        Bu_kinpt(6,current_node_idx+3) = B_deriv_global_kinpt(2,knode)
...
subroutine kbmatrix_vol(B_deriv_global_kinpt,ntens,nnode,ndim,Bu_vol_kinpt)
    ...
    do knode = 1, nnode
        ...
        Bu_vol_kinpt(1, current_node_idx+1) = B_deriv_global_kinpt(1, knode)
        Bu_vol_kinpt(1, current_node_idx+2) = B_deriv_global_kinpt(2, knode)
        Bu_vol_kinpt(1, current_node_idx+3) = B_deriv_global_kinpt(3, knode)
        Bu_vol_kinpt(2, current_node_idx+1) = B_deriv_global_kinpt(1, knode)
        Bu_vol_kinpt(2, current_node_idx+2) = B_deriv_global_kinpt(2, knode)
        Bu_vol_kinpt(2, current_node_idx+3) = B_deriv_global_kinpt(3, knode)
        Bu_vol_kinpt(3, current_node_idx+1) = B_deriv_global_kinpt(1, knode)
        Bu_vol_kinpt(3, current_node_idx+2) = B_deriv_global_kinpt(2, knode)
        Bu_vol_kinpt(3, current_node_idx+3) = B_deriv_global_kinpt(3, knode)
...
subroutine kstatevar(kinpt,nsvint,svars,statev,icopy)
...
!   material-point level state variable array.
    ...
    real(kind=dp), dimension(*) :: svars, statev

    isvinc = (kinpt-1) * nsvint     ! integration point increment

    if (icopy == 1) then ! Update current element statev from UEL svars
        do i = 1, nsvint
            statev(i) = svars(i+isvinc)
        end do
    else ! Update UEL svars from current element statev
        do i = 1, nsvint
...
subroutine UMAT_von_Mises(props,nprops,ddsdde,stress,stran,dstran, &
    ...
    ! material properties
    ! print *, 'start UMAT_von_Mises'
    ! print *, 'props = ', props(1:208)
...
subroutine UHARD_von_Mises(syield, hard, eqplas, statev, nvalue, table)
    ...
    real(kind=dp) :: syield, eqplas, deqplas, syiel0, syiel1, dsyiel, eqpl0, eqpl1
    real(kind=dp), dimension(3) :: hard
    real(kind=dp), dimension(*) :: statev
    real(kind=dp), dimension(2, nvalue) :: table
...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    ! Nodal displacement and incremental nodal displacement from previous increment
    real(kind=dp), dimension(ndim * nnode) :: u_prev, du_prev, du_prev_bar
    
    ! Shape function that interpolates from nodal points to IP at current IP for all nodes
    real(kind=dp), dimension(nnode) :: N_node_to_kinpt    

    ! Shape function that extrapolates from IP to nodal points at current node for all IPs
    real(kind=dp), dimension(ninpt) :: N_inpt_to_knode         
    
    ! Derivatives of N_node_to_kinpt with respect to isoparametric coordinates at current IP for all nodes
    real(kind=dp), dimension(ndim,nnode) :: B_deriv_local_kinpt           
    
    ! Jacobian and its inverse at current IP
    real(kind=dp), dimension(ndim,ndim) :: xjac, xjaci   

    ! Derivatives of N_node_to_kinpt with respect to global coordinates at current IP for all nodes
    real(kind=dp), dimension(ndim,nnode) :: B_deriv_global_kinpt          
                                                                    
    ! Average of B_deriv_global_kinpt over all IP for all nodes                                                        
    real(kind=dp), dimension(ndim,nnode) :: B_bar_deriv_global      

    ! Strain-displacement matrix at current IP for all nodes
    real(kind=dp), dimension(ntens,ndim*nnode) :: Bu_kinpt         

    ! Volumetric strain-displacement matrix at current IP for all nodes
    real(kind=dp), dimension(ntens,ndim*nnode) :: Bu_vol_kinpt    
    
    ! Deviatoric strain-displacement matrix at current IP for all nodes
    real(kind=dp), dimension(ntens,ndim*nnode) :: Bu_dev_kinpt          
    
    ! Average strain-displacement matrix over all IPs for all nodes
    real(kind=dp), dimension(ntens,ndim*nnode) :: Bu_bar          

    ! Average Volumetric strain-displacement matrix over all IPs
    real(kind=dp), dimension(ntens,ndim*nnode) :: Bu_bar_vol     
           

    !   Tangent stiffness matrix at current IP     
    real(kind=dp), dimension(ntens,ntens) :: ddsdde            
    !   Stress vector of the current element jelem at current IP     
    real(kind=dp), dimension(ntens) :: stress                       
    !   Strain vector of the current element jelem at current IP
    real(kind=dp), dimension(ntens) :: stran                        
    !   Incremental strain vector of the current element jelem at current IP
    real(kind=dp), dimension(ntens) :: dstran                      
    !   Elastic strain vector of the current element jelem at current IP
    real(kind=dp), dimension(ntens) :: eelas                        
    !   Plastic strain vector of the current element jelem at current IP
    real(kind=dp), dimension(ntens) :: eplas                        
    !   Local state variable of the current element jelem at current IP
    real(kind=dp), dimension(nsvint) :: statev
    

    ! Declaring all props as real(kind=dp) for consistency

    real(kind=dp) :: dvol, total_volume, djac, eqplas, deqplas, djac_bar, omega
...
!   initialising
    ...
    do kinpt=1, ninpt
        !   Compute N_node_to_kinpt and B_deriv_local_kinpt

        B_deriv_local_kinpt(1:ndim,1:nnode) = all_B_deriv_local_kinpt(kinpt,1:ndim,1:nnode) 

        !   Compute djac and B_deriv_global_kinpt
        call kjacobian(jelem,ndim,nnode,coords,B_deriv_local_kinpt,djac,B_deriv_global_kinpt,mcrd)
        ...
        if (kinc == 1) then
            ...
            print *, "coords"
            print *, coords
            print *, "B_deriv_local_kinpt"
            print *, B_deriv_local_kinpt
            print *, "B_deriv_global_kinpt"
            print *, B_deriv_global_kinpt
            print *, "dvol: ", dvol
        end if
    ...
    ! print *, "B_bar_deriv_global: "
    ! print *, B_bar_deriv_global
    ! print *, "total_volume: ", total_volume
    
    do kinpt=1,ninpt
        !   Compute N_node_to_kinpt and B_deriv_local_kinpt
        !  N_node_to_kinpt(1:nnode) = all_N_node_to_kinpt(kinpt,1:nnode)
        B_deriv_local_kinpt(1:ndim,1:nnode) = all_B_deriv_local_kinpt(kinpt,1:ndim,1:nnode) 
        ...
        !call kbmatrix_vol(B_bar_deriv_global,ntens,nnode,ndim,Bu_bar_vol)
