...
module common_block
    ...
    integer, parameter :: total_nodes = 20 ! Storing the actual number of nodes
    ...
    real(kind=dp) :: all_N_shape_int_to_knode(20, 8) ! (nnode, ninpt)
    real(kind=dp) :: all_N_shape_node_to_kinpt(8, 20) ! (ninpt, nnode)
    real(kind=dp) :: all_N_deriv_local_kinpt(8, 3, 20)  ! (ninpt, ndim, nnode)
...
module iso_module
    ...
    ! Isoparametric coordinates for nodal points in C3D20R element
    real(kind=dp), parameter :: xi_nodal_inter(20)   = (/ -coord_inter,  coord_inter,  coord_inter, -coord_inter, &
                                                          -coord_inter,  coord_inter,  coord_inter, -coord_inter, & 
                                                                 0.0d0,  coord_inter,        0.0d0, -coord_inter, &
                                                                 0.0d0,  coord_inter,        0.0d0, -coord_inter, &
                                                          -coord_inter,  coord_inter,  coord_inter, -coord_inter  &
                                                         /)
    real(kind=dp), parameter :: eta_nodal_inter(20)  = (/ -coord_inter, -coord_inter,  coord_inter,  coord_inter, &
                                                          -coord_inter, -coord_inter,  coord_inter,  coord_inter, & 
                                                          -coord_inter,        0.0d0,  coord_inter,        0.0d0, &
                                                          -coord_inter,        0.0d0,  coord_inter,        0.0d0, &
                                                          -coord_inter, -coord_inter,  coord_inter,  coord_inter  &
                                                         /)
    real(kind=dp), parameter :: zeta_nodal_inter(20) = (/ -coord_inter, -coord_inter, -coord_inter, -coord_inter, &
                                                           coord_inter,  coord_inter,  coord_inter,  coord_inter, & 
                                                          -coord_inter, -coord_inter, -coord_inter, -coord_inter, &
                                                           coord_inter,  coord_inter,  coord_inter,  coord_inter, &
    ...
    ! Isoparametric coordinates for integration points in C3D20R element
    ...
    real(kind=dp), parameter :: xi_nodal_extra(20)   = (/ -coord_extra,  coord_extra,  coord_extra, -coord_extra, &
                                                          -coord_extra,  coord_extra,  coord_extra, -coord_extra, & 
                                                                 0.0d0,  coord_extra,        0.0d0, -coord_extra, &
                                                                 0.0d0,  coord_extra,        0.0d0, -coord_extra, &
                                                          -coord_extra,  coord_extra,  coord_extra, -coord_extra  &
                                                         /)
    real(kind=dp), parameter :: eta_nodal_extra(20)  = (/ -coord_extra, -coord_extra,  coord_extra,  coord_extra, &
                                                          -coord_extra, -coord_extra,  coord_extra,  coord_extra, & 
                                                          -coord_extra,        0.0d0,  coord_extra,        0.0d0, &
                                                          -coord_extra,        0.0d0,  coord_extra,        0.0d0, &
                                                          -coord_extra, -coord_extra,  coord_extra,  coord_extra  &
                                                         /)
    real(kind=dp), parameter :: zeta_nodal_extra(20) = (/ -coord_extra, -coord_extra, -coord_extra, -coord_extra, &
                                                           coord_extra,  coord_extra,  coord_extra,  coord_extra, & 
                                                          -coord_extra, -coord_extra, -coord_extra, -coord_extra, &
                                                           coord_extra,  coord_extra,  coord_extra,  coord_extra, &
                                                                 0.0d0,        0.0d0,        0.0d0,        0.0d0  &
...
subroutine calc_N_shape_node_to_kinpt(xi_int, eta_int, zeta_int, N_shape_node_to_kinpt)
    ...
    use precision
    real(kind=dp), dimension(20) :: N_shape_node_to_kinpt ! nnode
    real(kind=dp) :: xi_int, eta_int, zeta_int

    !   shape functions


    !    N[1] := (1.-xi_int)*(1.-eta_int)*(1.-zeta_int)*(-xi_int-eta_int-zeta_int-2.)/8.:
    !    N[2] := (1.+xi_int)*(1.-eta_int)*(1.-zeta_int)*(xi_int-eta_int-zeta_int-2.)/8.:
    !    N[3] := (1.+xi_int)*(1.+eta_int)*(1.-zeta_int)*(xi_int+eta_int-zeta_int-2.)/8.:
    !    N[4] := (1.-xi_int)*(1.+eta_int)*(1.-zeta_int)*(-xi_int+eta_int-zeta_int-2.)/8.:
    !    N[5] := (1.-xi_int)*(1.-eta_int)*(1.+zeta_int)*(-xi_int-eta_int+zeta_int-2.)/8.:
    !    N[6] := (1.+xi_int)*(1.-eta_int)*(1.+zeta_int)*(xi_int-eta_int+zeta_int-2.)/8.:
    !    N[7] := (1.+xi_int)*(1.+eta_int)*(1.+zeta_int)*(xi_int+eta_int+zeta_int-2.)/8.:
    !    N[8] := (1.-xi_int)*(1.+eta_int)*(1.+zeta_int)*(-xi_int+eta_int+zeta_int-2.)/8.:
    !    N[9]  := (1.-xi_int^2)*(1.-eta_int)*(1.-zeta_int)/4.:
    !    N[10] := (1.+xi_int)*(1.-eta_int^2)*(1.-zeta_int)/4.:
    !    N[11] := (1.-xi_int^2)*(1.+eta_int)*(1.-zeta_int)/4.:
    !    N[12] := (1.-xi_int)*(1.-eta_int^2)*(1.-zeta_int)/4.:
    !    N[13] := (1.-xi_int^2)*(1.-eta_int)*(1.+zeta_int)/4.:
    !    N[14] := (1.+xi_int)*(1.-eta_int^2)*(1.+zeta_int)/4.:
    !    N[15] := (1.-xi_int^2)*(1.+eta_int)*(1.+zeta_int)/4.:
    !    N[16] := (1.-xi_int)*(1.-eta_int^2)*(1.+zeta_int)/4.:
    !    N[17] := (1.-xi_int)*(1.-eta_int)*(1.-zeta_int^2)/4.:
    !    N[18] := (1.+xi_int)*(1.-eta_int)*(1.-zeta_int^2)/4.:
    !    N[19] := (1.+xi_int)*(1.+eta_int)*(1.-zeta_int^2)/4.:
    !    N[20] := (1.-xi_int)*(1.+eta_int)*(1.-zeta_int^2)/4.:

    N_shape_node_to_kinpt(1) =  (1.0d0 - xi_int) * (1.0d0 - eta_int) * (1.0d0 - zeta_int) &
                                * (-xi_int - eta_int - zeta_int - 2.0d0) / 8.0d0
    N_shape_node_to_kinpt(2) =  (1.0d0 + xi_int) * (1.0d0 - eta_int) * (1.0d0 - zeta_int) &
                                * (xi_int - eta_int - zeta_int - 2.0d0) / 8.0d0
    N_shape_node_to_kinpt(3) =  (1.0d0 + xi_int) * (1.0d0 + eta_int) * (1.0d0 - zeta_int) &
                                * (xi_int + eta_int - zeta_int - 2.0d0) / 8.0d0
    N_shape_node_to_kinpt(4) =  (1.0d0 - xi_int) * (1.0d0 + eta_int) * (1.0d0 - zeta_int) &
                                * (-xi_int + eta_int - zeta_int - 2.0d0) / 8.0d0
    N_shape_node_to_kinpt(5) =  (1.0d0 - xi_int) * (1.0d0 - eta_int) * (1.0d0 + zeta_int) &
                                * (-xi_int - eta_int + zeta_int - 2.0d0) / 8.0d0
    N_shape_node_to_kinpt(6) =  (1.0d0 + xi_int) * (1.0d0 - eta_int) * (1.0d0 + zeta_int) &
                                * (xi_int - eta_int + zeta_int - 2.0d0) / 8.0d0
    N_shape_node_to_kinpt(7) =  (1.0d0 + xi_int) * (1.0d0 + eta_int) * (1.0d0 + zeta_int) &
                                * (xi_int + eta_int + zeta_int - 2.0d0) / 8.0d0
    N_shape_node_to_kinpt(8) =  (1.0d0 - xi_int) * (1.0d0 + eta_int) * (1.0d0 + zeta_int) &
                                * (-xi_int + eta_int + zeta_int - 2.0d0) / 8.0d0
    N_shape_node_to_kinpt(9) =  (1.0d0 - xi_int**2.0d0) * (1.0d0 - eta_int) * (1.0d0 - zeta_int) / 4.0d0
    N_shape_node_to_kinpt(10) = (1.0d0 + xi_int) * (1.0d0 - eta_int**2.0d0) * (1.0d0 - zeta_int) / 4.0d0
    N_shape_node_to_kinpt(11) = (1.0d0 - xi_int**2.0d0) * (1.0d0 + eta_int) * (1.0d0 - zeta_int) / 4.0d0
    N_shape_node_to_kinpt(12) = (1.0d0 - xi_int) * (1.0d0 - eta_int**2.0d0) * (1.0d0 - zeta_int) / 4.0d0
    N_shape_node_to_kinpt(13) = (1.0d0 - xi_int**2.0d0) * (1.0d0 - eta_int) * (1.0d0 + zeta_int) / 4.0d0
    N_shape_node_to_kinpt(14) = (1.0d0 + xi_int) * (1.0d0 - eta_int**2.0d0) * (1.0d0 + zeta_int) / 4.0d0
    N_shape_node_to_kinpt(15) = (1.0d0 - xi_int**2.0d0) * (1.0d0 + eta_int) * (1.0d0 + zeta_int) / 4.0d0
    N_shape_node_to_kinpt(16) = (1.0d0 - xi_int) * (1.0d0 - eta_int**2.0d0) * (1.0d0 + zeta_int) / 4.0d0
    N_shape_node_to_kinpt(17) = (1.0d0 - xi_int) * (1.0d0 - eta_int) * (1.0d0 - zeta_int**2.0d0) / 4.0d0
    N_shape_node_to_kinpt(18) = (1.0d0 + xi_int) * (1.0d0 - eta_int) * (1.0d0 - zeta_int**2.0d0) / 4.0d0
    N_shape_node_to_kinpt(19) = (1.0d0 + xi_int) * (1.0d0 + eta_int) * (1.0d0 - zeta_int**2.0d0) / 4.0d0
    N_shape_node_to_kinpt(20) = (1.0d0 - xi_int) * (1.0d0 + eta_int) * (1.0d0 - zeta_int**2.0d0) / 4.0d0
return
end
...
subroutine calc_N_deriv_local_kinpt(xi_int, eta_int, zeta_int, N_deriv_local_kinpt)
    ...
    use precision
    real(kind=dp), dimension(3, 20) :: N_deriv_local_kinpt ! (ndim, nnode)
    real(kind=dp) :: xi_int, eta_int, zeta_int


    N_deriv_local_kinpt(1,1) = (-(1.0d0-eta_int)*(1.0d0-zeta_int)*(-xi_int-eta_int-zeta_int-2.0d0) &
                                -(1.0d0-xi_int)*(1.0d0-eta_int)*(1.0d0-zeta_int))/8.0d0
    N_deriv_local_kinpt(2,1) = (-(1.0d0-xi_int)*(1.0d0-zeta_int)*(-xi_int-eta_int-zeta_int-2.0d0) &
                                -(1.0d0-xi_int)*(1.0d0-eta_int)*(1.0d0-zeta_int))/8.0d0
    N_deriv_local_kinpt(3,1) = (-(1.0d0-xi_int)*(1.0d0-eta_int)*(-xi_int-eta_int-zeta_int-2.0d0) &
                                -(1.0d0-xi_int)*(1.0d0-eta_int)*(1.0d0-zeta_int))/8.0d0

    N_deriv_local_kinpt(1,2) = ((1.0d0-eta_int)*(1.0d0-zeta_int)*(xi_int-eta_int-zeta_int-2.0d0) &
                                +(1.0d0+xi_int)*(1.0d0-eta_int)*(1.0d0-zeta_int))/8.0d0
    N_deriv_local_kinpt(2,2) = (-(1.0d0+xi_int)*(1.0d0-zeta_int)*(xi_int-eta_int-zeta_int-2.0d0) &
                                -(1.0d0+xi_int)*(1.0d0-eta_int)*(1.0d0-zeta_int))/8.0d0
    N_deriv_local_kinpt(3,2) = (-(1.0d0+xi_int)*(1.0d0-eta_int)*(xi_int-eta_int-zeta_int-2.0d0) &
                                -(1.0d0+xi_int)*(1.0d0-eta_int)*(1.0d0-zeta_int))/8.0d0

    N_deriv_local_kinpt(1,3) = ((1.0d0+eta_int)*(1.0d0-zeta_int)*(xi_int+eta_int-zeta_int-2.0d0) &
                                +(1.0d0+xi_int)*(1.0d0+eta_int)*(1.0d0-zeta_int))/8.0d0
    N_deriv_local_kinpt(2,3) = ((1.0d0+xi_int)*(1.0d0-zeta_int)*(xi_int+eta_int-zeta_int-2.0d0) &
                                +(1.0d0+xi_int)*(1.0d0+eta_int)*(1.0d0-zeta_int))/8.0d0
    N_deriv_local_kinpt(3,3) = (-(1.0d0+xi_int)*(1.0d0+eta_int)*(xi_int+eta_int-zeta_int-2.0d0) &
                                -(1.0d0+xi_int)*(1.0d0+eta_int)*(1.0d0-zeta_int))/8.0d0

    N_deriv_local_kinpt(1,4) = (-(1.0d0+eta_int)*(1.0d0-zeta_int)*(-xi_int+eta_int-zeta_int-2.0d0) &
                                -(1.0d0-xi_int)*(1.0d0+eta_int)*(1.0d0-zeta_int))/8.0d0
    N_deriv_local_kinpt(2,4) = ((1.0d0-xi_int)*(1.0d0-zeta_int)*(-xi_int+eta_int-zeta_int-2.0d0) &
                                +(1.0d0-xi_int)*(1.0d0+eta_int)*(1.0d0-zeta_int))/8.0d0
    N_deriv_local_kinpt(3,4) = (-(1.0d0-xi_int)*(1.0d0+eta_int)*(-xi_int+eta_int-zeta_int-2.0d0) &
                                -(1.0d0-xi_int)*(1.0d0+eta_int)*(1.0d0-zeta_int))/8.0d0
    
    N_deriv_local_kinpt(1,5) = (-(1.0d0-eta_int)*(1.0d0+zeta_int)*(-xi_int-eta_int+zeta_int-2.0d0) &
                                -(1.0d0-xi_int)*(1.0d0-eta_int)*(1.0d0+zeta_int))/8.0d0
    N_deriv_local_kinpt(2,5) = (-(1.0d0-xi_int)*(1.0d0+zeta_int)*(-xi_int-eta_int+zeta_int-2.0d0) &
                                -(1.0d0-xi_int)*(1.0d0-eta_int)*(1.0d0+zeta_int))/8.0d0
    N_deriv_local_kinpt(3,5) = ((1.0d0-xi_int)*(1.0d0-eta_int)*(-xi_int-eta_int+zeta_int-2.0d0) &
                                +(1.0d0-xi_int)*(1.0d0-eta_int)*(1.0d0+zeta_int))/8.0d0
    
    N_deriv_local_kinpt(1,6) = ((1.0d0-eta_int)*(1.0d0+zeta_int)*(xi_int-eta_int+zeta_int-2.0d0) &
                                +(1.0d0+xi_int)*(1.0d0-eta_int)*(1.0d0+zeta_int))/8.0d0
    N_deriv_local_kinpt(2,6) = (-(1.0d0+xi_int)*(1.0d0+zeta_int)*(xi_int-eta_int+zeta_int-2.0d0) &
                                -(1.0d0+xi_int)*(1.0d0-eta_int)*(1.0d0+zeta_int))/8.0d0
    N_deriv_local_kinpt(3,6) = ((1.0d0+xi_int)*(1.0d0-eta_int)*(xi_int-eta_int+zeta_int-2.0d0) &
                                +(1.0d0+xi_int)*(1.0d0-eta_int)*(1.0d0+zeta_int))/8.0d0
    
    N_deriv_local_kinpt(1,7) = ((1.0d0+eta_int)*(1.0d0+zeta_int)*(xi_int+eta_int+zeta_int-2.0d0) &
                                +(1.0d0+xi_int)*(1.0d0+eta_int)*(1.0d0+zeta_int))/8.0d0
    N_deriv_local_kinpt(2,7) = ((1.0d0+xi_int)*(1.0d0+zeta_int)*(xi_int+eta_int+zeta_int-2.0d0) &
                                +(1.0d0+xi_int)*(1.0d0+eta_int)*(1.0d0+zeta_int))/8.0d0
    N_deriv_local_kinpt(3,7) = ((1.0d0+xi_int)*(1.0d0+eta_int)*(xi_int+eta_int+zeta_int-2.0d0) &
                                +(1.0d0+xi_int)*(1.0d0+eta_int)*(1.0d0+zeta_int))/8.0d0
    
    N_deriv_local_kinpt(1,8) = (-(1.0d0+eta_int)*(1.0d0+zeta_int)*(-xi_int+eta_int+zeta_int-2.0d0) &
                                -(1.0d0-xi_int)*(1.0d0+eta_int)*(1.0d0+zeta_int))/8.0d0
    N_deriv_local_kinpt(2,8) = ((1.0d0-xi_int)*(1.0d0+zeta_int)*(-xi_int+eta_int+zeta_int-2.0d0) &
                                +(1.0d0-xi_int)*(1.0d0+eta_int)*(1.0d0+zeta_int))/8.0d0
    N_deriv_local_kinpt(3,8) = ((1.0d0-xi_int)*(1.0d0+eta_int)*(-xi_int+eta_int+zeta_int-2.0d0) &
                                +(1.0d0-xi_int)*(1.0d0+eta_int)*(1.0d0+zeta_int))/8.0d0
    
    N_deriv_local_kinpt(1,9)  = -2.0d0*xi_int*(1.0d0-eta_int)*(1.0d0-zeta_int)/4.0d0
    N_deriv_local_kinpt(2,9)  = -(1.0d0-xi_int**2.0d0)*(1.0d0-zeta_int)/4.0d0
    N_deriv_local_kinpt(3,9)  = -(1.0d0-xi_int**2.0d0)*(1.0d0-eta_int)/4.0d0
    
    N_deriv_local_kinpt(1,10)  = (1.0d0-eta_int**2.0d0)*(1.0d0-zeta_int)/4.0d0
    N_deriv_local_kinpt(2,10)  = -2.0d0*eta_int*(1.0d0+xi_int)*(1.0d0-zeta_int)/4.0d0
    N_deriv_local_kinpt(3,10)  = -(1.0d0-eta_int**2.0d0)*(1.0d0+xi_int)/4.0d0
    
    N_deriv_local_kinpt(1,11)  = -2.0d0*xi_int*(1.0d0+eta_int)*(1.0d0-zeta_int)/4.0d0
    N_deriv_local_kinpt(2,11)  = (1.0d0-xi_int**2.0d0)*(1.0d0-zeta_int)/4.0d0
    N_deriv_local_kinpt(3,11)  = -(1.0d0-xi_int**2.0d0)*(1.0d0+eta_int)/4.0d0
    
    N_deriv_local_kinpt(1,12)  = -(1.0d0-eta_int**2.0d0)*(1.0d0-zeta_int)/4.0d0
    N_deriv_local_kinpt(2,12)  = -2.0d0*eta_int*(1.0d0-xi_int)*(1.0d0-zeta_int)/4.0d0
    N_deriv_local_kinpt(3,12)  = -(1.0d0-eta_int**2.0d0)*(1.0d0-xi_int)/4.0d0
    
    N_deriv_local_kinpt(1,13)  = -2.0d0*xi_int*(1.0d0-eta_int)*(1.0d0+zeta_int)/4.0d0
    N_deriv_local_kinpt(2,13)  = -(1.0d0-xi_int**2.0d0)*(1.0d0+zeta_int)/4.0d0
    N_deriv_local_kinpt(3,13)  = (1.0d0-xi_int**2.0d0)*(1.0d0-eta_int)/4.0d0
    
    N_deriv_local_kinpt(1,14)  = (1.0d0-eta_int**2.0d0)*(1.0d0+zeta_int)/4.0d0
    N_deriv_local_kinpt(2,14)  = -2.0d0*eta_int*(1.0d0+xi_int)*(1.0d0+zeta_int)/4.0d0
    N_deriv_local_kinpt(3,14)  = (1.0d0-eta_int**2.0d0)*(1.0d0+xi_int)/4.0d0
    
    N_deriv_local_kinpt(1,15)  = -2.0d0*xi_int*(1.0d0+eta_int)*(1.0d0+zeta_int)/4.0d0
    N_deriv_local_kinpt(2,15)  =  (1.0d0-xi_int**2.0d0)*(1.0d0+zeta_int)/4.0d0
    N_deriv_local_kinpt(3,15)  = (1.0d0-xi_int**2.0d0)*(1.0d0+eta_int)/4.0d0
    
    N_deriv_local_kinpt(1,16)  = -(1.0d0-eta_int**2.0d0)*(1.0d0+zeta_int)/4.0d0
    N_deriv_local_kinpt(2,16)  = -2.0d0*eta_int*(1.0d0-xi_int)*(1.0d0+zeta_int)/4.0d0
    N_deriv_local_kinpt(3,16)  = (1.0d0-eta_int**2.0d0)*(1.0d0-xi_int)/4.0d0
    
    N_deriv_local_kinpt(1,17) = -(1.0d0-eta_int)*(1.0d0-zeta_int**2.0d0)/4.0d0
    N_deriv_local_kinpt(2,17) = -(1.0d0-xi_int)*(1.0d0-zeta_int**2.0d0)/4.0d0
    N_deriv_local_kinpt(3,17) = -zeta_int*(1.0d0-xi_int)*(1.0d0-eta_int)/2.0d0
    
    N_deriv_local_kinpt(1,18) = (1.0d0-eta_int)*(1.0d0-zeta_int**2.0d0)/4.0d0
    N_deriv_local_kinpt(2,18) = -(1.0d0+xi_int)*(1.0d0-zeta_int**2.0d0)/4.0d0
    N_deriv_local_kinpt(3,18) = -zeta_int*(1.0d0+xi_int)*(1.0d0-eta_int)/2.0d0
    
    N_deriv_local_kinpt(1,19) = (1.0d0+eta_int)*(1.0d0-zeta_int**2.0d0)/4.0d0
    N_deriv_local_kinpt(2,19) = (1.0d0+xi_int)*(1.0d0-zeta_int**2.0d0)/4.0d0
    N_deriv_local_kinpt(3,19) = -zeta_int*(1.0d0+xi_int)*(1.0d0+eta_int)/2.0d0
    
    N_deriv_local_kinpt(1,20) = -(1.0d0+eta_int)*(1.0d0-zeta_int**2.0d0)/4.0d0
    N_deriv_local_kinpt(2,20) = (1.0d0-xi_int)*(1.0d0-zeta_int**2.0d0)/4.0d0
    N_deriv_local_kinpt(3,20) = -zeta_int*(1.0d0-xi_int)*(1.0d0+eta_int)/2.0d0


    ! Above is referenced code for C3D20R shape function derivation, dNdxi is shape(nnode, ndim)
    ! However now we want to translate this to fortran with shape (ndim, nnode)
...
subroutine UEXTERNALDB(lop,lrestart,time,dtime,kstep,kinc)
    ...
    if (lop == 0) then 
        ...
        do knode = 1, nnode
            ...
            all_N_shape_int_to_knode(knode, 1:ninpt) = N_shape_int_to_knode
...
subroutine calc_Bu_bar(N_deriv_global_kinpt,N_bar_deriv_global, &
                            ntens,nnode,ndim,Bu_bar)
    use precision
    include 'aba_param.inc'

    real(kind=dp), dimension(ndim,nnode) :: N_deriv_global_kinpt
    real(kind=dp), dimension(ndim,nnode) :: N_bar_deriv_global
    real(kind=dp), dimension(ntens,ndim*nnode) :: Bu_bar  

    integer :: current_node_idx
    real(kind=dp) :: c4, c5, c6, c7, c8, c9
    ...
    do knode = 1, nnode
        ...
        c4 = (1.0d0/3.0d0) * (N_bar_deriv_global(1, knode) - N_deriv_global_kinpt(1, knode))
        c5 = N_deriv_global_kinpt(1, knode) + c4
        c6 = (1.0d0/3.0d0) * (N_bar_deriv_global(2, knode) - N_deriv_global_kinpt(2, knode))
        c7 = N_deriv_global_kinpt(2, knode) + c6
        c8 = (1.0d0/3.0d0) * (N_bar_deriv_global(3, knode) - N_deriv_global_kinpt(3, knode))
        c9 = N_deriv_global_kinpt(3, knode) + c8

        ! Volumetric components
        Bu_bar(1, current_node_idx+1) = c5
        Bu_bar(1, current_node_idx+2) = c6
        Bu_bar(1, current_node_idx+3) = c8
        Bu_bar(2, current_node_idx+1) = c4
        Bu_bar(2, current_node_idx+2) = c7
        Bu_bar(2, current_node_idx+3) = c8
        Bu_bar(3, current_node_idx+1) = c4
        Bu_bar(3, current_node_idx+2) = c6
        Bu_bar(3, current_node_idx+3) = c9
        ! Deviatoric component
        Bu_bar(4,current_node_idx+1) = 0.0d0
        Bu_bar(4,current_node_idx+2) = N_deriv_global_kinpt(3,knode)
        Bu_bar(4,current_node_idx+3) = N_deriv_global_kinpt(2,knode)

        Bu_bar(5,current_node_idx+1) = N_deriv_global_kinpt(3,knode)
        Bu_bar(5,current_node_idx+2) = 0.0d0
        Bu_bar(5,current_node_idx+3) = N_deriv_global_kinpt(1,knode)

        Bu_bar(6,current_node_idx+1) = N_deriv_global_kinpt(2,knode)
        Bu_bar(6,current_node_idx+2) = N_deriv_global_kinpt(1,knode)
        Bu_bar(6,current_node_idx+3) = 0.0d0
...
subroutine calc_sym_matrix(nsize, matrix, matrix_sym)
    use precision
    include 'aba_param.inc'
    real(kind=dp), dimension(nsize,nsize) :: matrix, matrix_sym
...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    integer, parameter :: end_u_idx = 60 ! = ndim * nnode = 3 x 20 = 60 
    
    ! The following data is not part of UEL, defined by the user    
    real(kind=dp), dimension(ndim * nnode) :: u_current, du_current
    ...
    real(kind=dp), dimension(ntens) :: stran, dstran                       ! Incremental strain vector of the current element jelem
    ...
    real(kind=dp) :: djac, eqplas, deqplas, djac_bar, omega_stran, omega_dstran, vol_el
    ...
    integer :: u_current_index_idim, u_current_index_jdim
...
!   initialising
    ...
    do kinpt=1, ninpt
        ...
        !     omega_stran = omega_stran + eps_vol * dvol
        ! end if
    ...
    N_bar_deriv_global = N_bar_deriv_global / vol_el
    ! N_bar_deriv_global = N_bar_deriv_global / (3.0d0 * vol_el)
    djac_bar = djac_bar / vol_el

    do knode=1, nnode
        do kdim=1, ndim
            omega_stran = omega_stran + N_bar_deriv_global(kdim,knode) * u_current(ndim * knode - ndim + kdim)
            omega_dstran = omega_dstran + N_bar_deriv_global(kdim,knode) * du_current(ndim * knode - ndim + kdim)
        end do
    end do

    ! print *, 'omega_stran = ', omega_stran
    ...
    !     print *, 'omega_stran = ', omega_stran
    !     print *, 'vol_el = ', vol_el
    !     print *, 'N_bar_deriv_global = ', N_bar_deriv_global
    !     print *, 'u_current = ', u_current
    !     print *, 'du_current = ', du_current
    ! end if
    ...
    do kinpt=1,ninpt
            ...
            do jdim = 1, ndim
                do knode = 1, nnode
                    ...
                    stran_tensor(idim, jdim) = stran_tensor(idim, jdim) + &
                        0.5d0 * (u_current(u_current_index_idim) * N_deriv_global_kinpt(jdim, knode) + &
                                u_current(u_current_index_jdim) * N_deriv_global_kinpt(idim, knode))
        ...
        ! ! call calc_Bu_bar(N_deriv_global_kinpt,N_bar_deriv_global, &
        ! !                   ntens,nnode,ndim,Bu_bar)
        ...
        ! du_current_bar = du_current * (djac_bar/djac)
