...
!      is included to make generalizing the element to nonlinear materials easier)
        ...
        do idim = 1,3
            do jdim = 1,3
                do knode = 1,nnode
                    stran(idim,jdim) = stran(idim,jdim)   + 0.5d0*(u_current(idim,knode)*N_deriv_global_kinpt(knode,jdim) + u_current(jdim,knode)*N_deriv_global_kinpt(knode,inode))
                    dstran(idim,jdim) = dstran(idim,jdim) + 0.5d0*(du_current(idim,knode)*N_deriv_global_kinpt(knode,jdim) + du_current(jdim,knode)*N_deriv_global_kinpt(knode,inode))
                end do
            end do
        end do
!
