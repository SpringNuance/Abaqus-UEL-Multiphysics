...
def return_description_properties(properties_path_excel, flow_curve_excel_path):
    ...
    mass_diffusion_descriptions_list = properties_df["mass_diffusion_descriptions"].dropna().tolist()
    mass_diffusion_values_list = properties_df["mass_diffusion_values"].dropna().tolist()
    ...
    description_properties_dict["mass_diffusion_properties"] = dict(zip(mass_diffusion_descriptions_list, mass_diffusion_values_list))
...
def return_UEL_property(description_properties_dict): 
    ...
    mass_diffusion_properties_list = list(description_properties_dict["mass_diffusion_properties"].values())
    mass_diffusion_description_list = list(description_properties_dict["mass_diffusion_properties"].keys())
    ...
    mass_diffusion_prop_num_lines = int(np.ceil(len(mass_diffusion_properties_list)/8))
    mass_diffusion_prop_num_properties = int(mass_diffusion_prop_num_lines*8)
    ...
    total_num_properties = mech_prop_num_properties + flow_curve_num_properties + heat_transfer_prop_num_properties +\
                        mass_diffusion_prop_num_properties + phase_field_prop_num_properties
    ...
    # For mass diffusion properties
    UEL_property.append("**")
    UEL_property.append("** =============================")
    UEL_property.append("**")
    UEL_property.append("** MASS DIFFUSION PROPERTIES")
    UEL_property.append("**")

    for line_index in range(mass_diffusion_prop_num_lines):
        if line_index != mass_diffusion_prop_num_lines - 1:
            subset_properties = mass_diffusion_properties_list[line_index*8:(line_index+1)*8]
            subset_description = mass_diffusion_description_list[line_index*8:(line_index+1)*8]
            UEL_property.append("** " + ", ".join(subset_description[0:4]))
            UEL_property.append("** " + ", ".join(subset_description[4:8]))
            UEL_property.append(", ".join(subset_properties))
            
        else:
            subset_properties = mass_diffusion_properties_list[line_index*8:] + ["0.0"]*(8-len(mass_diffusion_properties_list[line_index*8:]))
            subset_description = mass_diffusion_description_list[line_index*8:] + ["none"]*(8-len(mass_diffusion_description_list[line_index*8:]))
            UEL_property.append("** " + ", ".join(subset_description[0:4]))
            UEL_property.append("** " + ", ".join(subset_description[4:8]))
            UEL_property.append(", ".join(subset_properties))
