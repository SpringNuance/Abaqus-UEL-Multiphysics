...
! CPE4R/CPS4R/CPE4RT/CPS4RT element
...
module iso_module_CPE4
    ...
    real(kind=dp), parameter :: xi_node_inter_CPE4(4)    = (/ -1.0d0,  1.0d0,  1.0d0, -1.0d0 /)
    real(kind=dp), parameter :: eta_node_inter_CPE4(4)   = (/ -1.0d0, -1.0d0,  1.0d0,  1.0d0 /)

    ! Isoparametric coordinates of IPs. Shape (ninpt)
    
    real(kind=dp), parameter :: xi_inpt_inter_CPE4(4)    = (/ -0.57735d0,  0.57735d0, -0.57735d0,  0.57735d0 /)
    real(kind=dp), parameter :: eta_inpt_inter_CPE4(4)   = (/ -0.57735d0, -0.57735d0,  0.57735d0,  0.57735d0 /)

    ! Extrapolating coordinates (IPs out to other local coordinates, such as NPs)

    ! Isoparametric coordinates of NPs. Shape (nnode)

    real(kind=dp), parameter :: xi_node_extra_CPE4(4)    = (/ -1.73205d0,  1.73205d0,  1.73205d0, -1.73205d0 /)
    real(kind=dp), parameter :: eta_node_extra_CPE4(4)   = (/ -1.73205d0, -1.73205d0,  1.73205d0,  1.73205d0 /)

    ! Isoparametric coordinates of IPs. Shape (ninpt)

    real(kind=dp), parameter :: xi_inpt_extra_CPE4(4)   = (/ -1.0d0,  1.0d0, -1.0d0,  1.0d0 /)
    real(kind=dp), parameter :: eta_inpt_extra_CPE4(4)  = (/ -1.0d0, -1.0d0,  1.0d0,  1.0d0 /)


end module iso_module_CPE4

subroutine calc_N_inpt_to_local_coords_CPE4(xi_coord, eta_coord, N_inpt_to_local_coords)
    ...
    N_inpt_to_local_coords(1) = 0.25d0 * (1.0d0 - xi_coord) * (1.0d0 - eta_coord)
    N_inpt_to_local_coords(2) = 0.25d0 * (1.0d0 + xi_coord) * (1.0d0 - eta_coord)
    N_inpt_to_local_coords(3) = 0.25d0 * (1.0d0 - xi_coord) * (1.0d0 + eta_coord)
    N_inpt_to_local_coords(4) = 0.25d0 * (1.0d0 + xi_coord) * (1.0d0 + eta_coord)

return
end

subroutine calc_N_node_to_local_coords_CPE4(xi_coord, eta_coord, &
...
subroutine calc_N_grad_node_to_local_coords_CPE4(xi_coord, eta_coord, &
