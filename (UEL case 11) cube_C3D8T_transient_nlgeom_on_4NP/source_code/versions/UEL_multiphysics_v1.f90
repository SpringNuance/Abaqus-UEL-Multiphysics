...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    real(kind=dp) :: u_heat_t, u_heat_tm1, dudt_heat, du_heat, ru_heat
    ...
    real(kind=dp), dimension(nnode,nnode)  :: M_heat_capacity, K_heat_conductivity
    real(kind=dp), dimension(nnode,nnode) :: M_dudt_heat_term, M_dudg_heat_term, K_dfdt_heat_term, K_dfdg_heat_term
    real(kind=dp), dimension(nnode) :: F_heat_rhs, F_ru_heat_term, F_dudt_heat_term, F_flux_heat_term
    ...
    do kinpt=1,ninpt
        ...
        ! temp_kIP_tm1, dtemp_kIP, temp_grad_kIP_t, u_heat are all defined at integration points
        ...
        ! Then we also have the remaining value, which is ru_heat = du_heat - dudt_heat
        
        u_heat_t = u_heat ! u_heat is updated as u_heat_t
        statev(u_heat_idx) = u_heat

        if (kinc == 1) then
            print *, 'u_heat_t = ', u_heat_t
            print *, 'u_heat_tm1 = ', u_heat_tm1
            print *, 'dudt_heat = ', dudt_heat
            print *, 'dudg_heat = ', dudg_heat
            print *, 'dfdt_heat = ', dfdt_heat
            print *, 'dfdg_heat = ', dfdg_heat
            print *, 'flux_heat = ', flux_heat
        end if
        du_heat = u_heat_t - u_heat_tm1
        ru_heat = du_heat - dudt_heat * dtemp_kIP


        ! Matrix multiplication is associative 
        
        ! These terms dfdt and dfdg are for the K_heat_conductivity matrix

        ! (nnode, nnode) = (nnode, ndim) @ (ndim, 1) @ (1, nnode)
        K_dfdt_heat_term    = - matmul( &
                                        transpose(N_grad_NP_to_kIP_global_t), & 
                                        matmul(reshape(dfdt_heat, [ndim, 1]), & 
                                        reshape(N_shape_NP_to_kIP, [1, nnode])) &
                                        )

        ! (nnode, nnode) = (nnode, ndim) @ (ndim, ndim) @ (ndim, nnode)
        K_dfdg_heat_term    = - matmul(transpose(N_grad_NP_to_kIP_global_t), matmul(dfdg_heat, N_grad_NP_to_kIP_global_t))
        
        K_heat_conductivity = K_dfdt_heat_term + K_dfdg_heat_term

        ! These terms dudt and dudg are for the M_heat_capacity matrix

        ! (nnode, nnode) = (nnode, 1) @ (1, nnode)
        M_dudt_heat_term    = + matmul( &
                                        reshape(N_shape_NP_to_kIP, [nnode, 1]) * rho_heat * dudt_heat, &
                                        reshape(N_shape_NP_to_kIP, [1, nnode]) &
                                        ) 
        
        ! (nnode, nnode) = (nnode, 1) @ (1, ndim) @ (ndim, nnode)
        M_dudg_heat_term    = + matmul( &
                                        reshape(N_shape_NP_to_kIP, [nnode, 1]), &
                                        matmul(reshape(rho_heat * dudg_heat, [1, ndim]), &
                                               N_grad_NP_to_kIP_global_t))
        
        M_heat_capacity = M_dudt_heat_term + M_dudg_heat_term

        ! (nnode) = (nnode) * scalar * scalar
        F_dudt_heat_term = - N_shape_NP_to_kIP * rho_heat * dudt_heat * (dtemp_kIP/dtime)
        
        ! (nnode) = (nnode) * scalar 
        F_ru_heat_term   = - N_shape_NP_to_kIP * rho_heat * ru_heat
        
        ! (nnode) = (nnode, ndim) @ (ndim)
        F_flux_heat_term      = + matmul(transpose(N_grad_NP_to_kIP_global_t), flux_heat)
        
        F_heat_rhs = F_dudt_heat_term + F_ru_heat_term + F_flux_heat_term

        ! print *, 'M_heat_capacity = ', M_heat_capacity
        ! print *, 'K_heat_conductivity = ', K_heat_conductivity
        ! print *, 'F_heat_rhs = ', F_heat_rhs
        
        ! Galerkin representation of the transient heat transfer equation
        ! K_heat_conductivity * dtemp_NPs_t + M_heat_capacity * dtemp_NPs_t/dtemp = F_heat_rhs
        ! => (K_heat_conductivity * M_heat_capacity /dtime) * dtemp_NPs_t = F_heat_rhs
        ...
        amatrx(start_temp_idx:end_temp_idx,start_temp_idx:end_temp_idx) = &
            amatrx(start_temp_idx:end_temp_idx,start_temp_idx:end_temp_idx) &
            + dvol_t * (K_heat_conductivity + M_heat_capacity/dtime)
