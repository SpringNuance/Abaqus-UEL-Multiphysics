...
subroutine build_elems_to_NPs_matrix()
    use precision
    use common_block
    ...
    ! Read the file line by line and populate the NPs_to_elems_matrix
    do line_idx = 1, total_elems
        ...
        ! Populate the elems_to_NPs_matrix

        do knode = 2, nnode + 1 ! Looping over the 8 nodes
            element_ID = values(1)
            node_ID = values(knode)
            elems_to_NPs_matrix(element_ID, knode-1) = node_ID
        end do
    end do
...
subroutine build_NPs_to_elems_matrix()
    use precision
    use common_block
    include 'aba_param.inc'
    ...
    ! Initialize NPs_to_elems_matrix with 0 to indicate unused slots
    NPs_to_elems_matrix = 0
    nelems_of_NPs_matrix = 0
    ...
    do line_idx = 1, total_elems
        ...
        ! Populating the NPs_to_elems_matrix

        element_ID = values(1)
            
        ! Loop over each node in the current element
        do knode = 1, nnode  ! nnode is the number of nodes per element
            node_ID = elems_to_NPs_matrix(element_ID, knode)  ! Get the global node number

            nelems_of_NPs_matrix(node_ID) = nelems_of_NPs_matrix(node_ID) + 1
            kelem = nelems_of_NPs_matrix(node_ID)  ! Get the current index for element containing this node

            ! Store the element ID and local node number in NPs_to_elems_matrix
            NPs_to_elems_matrix(node_ID, kelem, 1) = element_ID  ! Store the element ID
            NPs_to_elems_matrix(node_ID, kelem, 2) = knode  ! Store the local node number
