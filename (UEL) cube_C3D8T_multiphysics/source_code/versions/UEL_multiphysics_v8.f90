...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    do kinpt=1,ninpt
        ...
        K_dfdg_heat_term_kIP_t    = - matmul(transpose(N_grad_NP_inter_to_kIP_extra_global_t), & 
                                             matmul(dfdg_heat_kIP_t, N_grad_NP_inter_to_kIP_extra_global_t))
        ...
        F_du_heat_term_kIP_t = + N_shape_NP_inter_to_kIP_extra * rho_heat * du_heat_kIP

        ! (nnode) = (nnode, ndim) @ (ndim)
        F_flux_heat_term_kIP_t  = - matmul(transpose(N_grad_NP_inter_to_kIP_extra_global_t), flux_heat_kIP_t)
        ...
        if (lflags(1) == 31 .or. lflags(1) == 71) then
            M_heat_capacitance_kIP_t = 0.0d0
            F_du_heat_term_kIP_t = 0.0d0
        end if 

        amatrx(start_temp_idx:end_temp_idx,start_temp_idx:end_temp_idx) = &
            amatrx(start_temp_idx:end_temp_idx,start_temp_idx:end_temp_idx) &
            + dvol_inter_t * (K_heat_conductivity_kIP_t + M_heat_capacitance_kIP_t/dtime)
            
        rhs(start_temp_idx:end_temp_idx,nrhs) = rhs(start_temp_idx:end_temp_idx,nrhs) &
            - dvol_inter_t * (F_du_heat_term_kIP_t + F_flux_heat_term_kIP_t/dtime)
