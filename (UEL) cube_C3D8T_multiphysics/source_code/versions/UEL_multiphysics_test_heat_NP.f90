...
subroutine UEL(rhs,amatrx,svars,energy,ndofel,nrhs,nsvars, &
    ...
    do kinpt=1,ninpt
        ...
        ! In Abaqus linear elements, the temperature integration are actually located at nodal points, not normal Gauss integration points!
        ...
        temp_kIP_tm1 = dot_product(N_shape_NP_inter_to_kIP_extra, temp_NPs_tm1)
        temp_kIP_t = dot_product(N_shape_NP_inter_to_kIP_extra, temp_NPs_t)
        dtemp_kIP = temp_kIP_t - temp_kIP_tm1
        temp_grad_kIP_t = matmul(N_grad_NP_inter_to_kIP_extra_global_t, temp_NPs_t)
        
        rho_heat = props(start_temp_props_idx)

        ! The internal energy per unit mass
        u_heat_kIP = statev(u_heat_idx)
        u_heat_kIP = matmul(N_shape_NP_inter_to_kIP_extra, u_heat_NPs)
        ...
        ! (nnode, nnode) = (nnode, ndim) @ (ndim, ndim) @ (ndim, nnode)
        K_heat_dfdg_kIP_t   = - matmul(transpose(N_grad_NP_inter_to_kIP_extra_global_t), & 
        ...
        ! (nnode) = (nnode, ndim) @ (ndim)
        F_heat_flux_kIP_t  = - matmul(transpose(N_grad_NP_inter_to_kIP_extra_global_t), flux_heat_kIP_t)
        ...
        !if (kinc > 1) then
            amatrx(start_temp_idx:end_temp_idx,start_temp_idx:end_temp_idx) = &
                amatrx(start_temp_idx:end_temp_idx,start_temp_idx:end_temp_idx) &
                + dvol_extra_t * (K_heat_kIP_t + M_heat_kIP_t/dtime)
                
            rhs(start_temp_idx:end_temp_idx,nrhs) = rhs(start_temp_idx:end_temp_idx,nrhs) &
                - dvol_extra_t * (F_heat_r_kIP_t + F_heat_flux_kIP_t + F_heat_du_kIP_t/dtime)
        !end if
