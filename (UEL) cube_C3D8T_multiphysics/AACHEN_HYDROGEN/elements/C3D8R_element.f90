...
! C3D8RR/C3D8RRT element

!*****************************************************************
!  8 nodal points, 1 integration point 3D hexahedral element
! 
!             8---------------7
!            /|              /|       zeta (positive)
!           / |             / |       
...
module iso_module_C3D8R
    ...
    real(kind=dp), parameter :: xi_node_inter_C3D8R(8)    = (/ -1.0d0,  1.0d0,  1.0d0, -1.0d0, &
                                                               -1.0d0,  1.0d0,  1.0d0, -1.0d0 /)
    real(kind=dp), parameter :: eta_node_inter_C3D8R(8)   = (/ -1.0d0, -1.0d0,  1.0d0,  1.0d0, &
                                                               -1.0d0, -1.0d0,  1.0d0,  1.0d0 /)
    real(kind=dp), parameter :: zeta_node_inter_C3D8R(8)  = (/ -1.0d0, -1.0d0, -1.0d0, -1.0d0, &
                                                                1.0d0,  1.0d0,  1.0d0,  1.0d0 /)

    ! Isoparametric coordinates of IPs. Shape (ninpt)
    
    ! Isoparametric coordinates of Integration Points (Single Gauss Point at Center)
    real(kind=dp), parameter :: xi_inpt_inter_C3D8R(1)    = (/ 0.0d0 /)
    real(kind=dp), parameter :: eta_inpt_inter_C3D8R(1)   = (/ 0.0d0 /)
    real(kind=dp), parameter :: zeta_inpt_inter_C3D8R(1)  = (/ 0.0d0 /)

    ! Extrapolating coordinates (IPs out to other local coordinates, such as NPs)

    ! Isoparametric coordinates of NPs. Shape (nnode)

    real(kind=dp), parameter :: xi_node_extra_C3D8R(8)    = (/ -1.73205d0,  1.73205d0,  1.73205d0, -1.73205d0, &
                                                               -1.73205d0,  1.73205d0,  1.73205d0, -1.73205d0 /)
    real(kind=dp), parameter :: eta_node_extra_C3D8R(8)   = (/ -1.73205d0, -1.73205d0,  1.73205d0,  1.73205d0, &
                                                               -1.73205d0, -1.73205d0,  1.73205d0,  1.73205d0 /)
    real(kind=dp), parameter :: zeta_node_extra_C3D8R(8)  = (/ -1.73205d0, -1.73205d0, -1.73205d0, -1.73205d0, &
                                                                1.73205d0,  1.73205d0,  1.73205d0,  1.73205d0 /)

    ! Isoparametric coordinates of IPs. Shape (ninpt)

    real(kind=dp), parameter :: xi_inpt_extra_C3D8R(1)   = (/ 0.0d0 /)
    real(kind=dp), parameter :: eta_inpt_extra_C3D8R(1)  = (/ 0.0d0 /)
    real(kind=dp), parameter :: zeta_inpt_extra_C3D8R(1) = (/ 0.0d0 /)

end module iso_module_C3D8R

subroutine calc_N_inpt_to_local_coords_C3D8R(xi_coord, eta_coord, zeta_coord, N_inpt_to_local_coords)
    ...
    real(kind=dp), dimension(1) :: N_inpt_to_local_coords ! Shape (ninpt)
...
subroutine calc_N_node_to_local_coords_C3D8R(xi_coord, eta_coord, zeta_coord, N_node_to_local_coords)
...
subroutine calc_N_grad_node_to_local_coords_C3D8R(xi_coord, eta_coord, zeta_coord, &
