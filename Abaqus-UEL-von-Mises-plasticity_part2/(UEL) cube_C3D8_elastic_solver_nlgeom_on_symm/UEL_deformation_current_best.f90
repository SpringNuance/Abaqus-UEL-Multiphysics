...
!   initialising
    ...
    do kinpt=1,ninpt
        ...
        if (lflags(2) == 0) then 
            ...
            ! =======================================
            stran_tm1 = matmul(Bu_bar_tm1, ux_flat_tm1)
            ...
            ! Engineering/infinitesimal strain (reduced volumetric strain)
            ! ========================================

            ! stran_omega_tm1 = 0.0d0
            ! dstran_omega = 0.0d0

            ! do knode=1, nnode
            !     do kdim=1, ndim
            !         stran_omega_tm1 = stran_omega_tm1 + N_bar_deriv_global_t(kdim,knode) * ux_tm1(kdim,knode)
            !         dstran_omega = dstran_omega + N_bar_deriv_global_t(kdim,knode) * dux(kdim,knode)
            !     end do
            ! end do
            ...
            ! stran_tensor_tm1 = 0.0d0
            ! dstran_tensor = 0.0d0
            
            ! do idim = 1, ndim
            !     do jdim = 1, ndim
            !         do knode = 1, nnode
            !             stran_tensor_tm1(idim, jdim) = stran_tensor_tm1(idim, jdim) + &
            !                 0.5d0 * (ux_tm1(idim,knode) * N_bar_deriv_global_tm1(jdim, knode) + &
            !                         ux_tm1(jdim,knode) * N_bar_deriv_global_tm1(idim, knode))
            !             dstran_tensor(idim, jdim) = dstran_tensor(idim, jdim) + &
            !                 0.5d0 * (dux(idim,knode) * N_bar_deriv_global_t(jdim, knode) + &
            !                         dux(jdim,knode) * N_bar_deriv_global_t(idim, knode))
            !         end do
            !     end do
            !     stran_vol_tm1 = stran_vol_tm1 + stran_tensor_tm1(idim, idim)
            !     dstran_vol = dstran_vol + dstran_tensor(idim, idim)
            ! end do

            ! ! 3.0d0 is ndim but in double precision
            ! stran_tensor_tm1(1,1) = stran_tensor_tm1(1,1) - stran_vol_tm1/3.0d0 + stran_omega_tm1/3.0d0
            ! stran_tensor_tm1(2,2) = stran_tensor_tm1(2,2) - stran_vol_tm1/3.0d0 + stran_omega_tm1/3.0d0
            ! stran_tensor_tm1(3,3) = stran_tensor_tm1(3,3) - stran_vol_tm1/3.0d0 + stran_omega_tm1/3.0d0
            
            ! dstran_tensor(1,1) = dstran_tensor(1,1) - dstran_vol/3.0d0 + dstran_omega/3.0d0
            ! dstran_tensor(2,2) = dstran_tensor(2,2) - dstran_vol/3.0d0 + dstran_omega/3.0d0
            ! dstran_tensor(3,3) = dstran_tensor(3,3) - dstran_vol/3.0d0 + dstran_omega/3.0d0

            ! stran_tm1(1) = stran_tensor_tm1(1,1)
            ! stran_tm1(2) = stran_tensor_tm1(2,2)
            ! stran_tm1(3) = stran_tensor_tm1(3,3)
            ! stran_tm1(4) = stran_tensor_tm1(1,2) * 2.0d0
            ! stran_tm1(5) = stran_tensor_tm1(1,3) * 2.0d0
            ! stran_tm1(6) = stran_tensor_tm1(2,3) * 2.0d0
            ...
            call calc_u_grad(dux_grad, dux, N_deriv_global_kinpt_t, ndim, nnode)

            ! call calc_u_grad(ux_grad_t, ux_t, N_bar_deriv_global_t, ndim, nnode)
            ! call calc_u_grad(ux_grad_tm1, ux_tm1, N_bar_deriv_global_tm1, ndim, nnode)
            ! call calc_u_grad(dux_grad, dux, N_bar_deriv_global_t, ndim, nnode)
            ...
            ! dF_grad_bar = matmul(F_grad_bar_t, F_grad_bar_inv_tm1)
            ! dF_grad_bar = matmul(F_grad_bar_inv_tm1, F_grad_bar_t)
            
            ! === STEP 2: Perform polar decomposition to separate rotation and stretch ===
            ...
            ! === STEP 3: Compute rotation-free incremental logarithmic strain ===
            call calc_matrix_log(dV_stretch, deps_log, ndim)

            ! === Compute total logarithmic strain at end of increment (t) ===
            B_Cauchy_t = matmul(F_grad_bar_t, transpose(F_grad_bar_t))
            call calc_matrix_sqrt(B_Cauchy_t, V_stretch_t, ndim)
            call calc_matrix_log(V_stretch_t, eps_log_t, ndim)

            ! === Compute total logarithmic strain at start of increment (t-1) ===
            B_Cauchy_tm1 = matmul(F_grad_bar_tm1, transpose(F_grad_bar_tm1))
            call calc_matrix_sqrt(B_Cauchy_tm1, V_stretch_tm1, ndim)
            call calc_matrix_log(V_stretch_tm1, eps_log_tm1, ndim)

            ! deps_log = eps_log_t - eps_log_tm1


            ! dF_grad_bar = dF_grad * (djac_bar_t / djac_t) ** (1.0d0/3.0d0)

            ! ! Right Cauchy stress tensor
            ! C_Cauchy_t = matmul(transpose(F_grad_bar_t), F_grad_bar_t)
            ! C_Cauchy_tm1 = matmul(transpose(F_grad_bar_tm1), F_grad_bar_tm1)
            ! dC_Cauchy = matmul(transpose(dF_grad_bar), dF_grad_bar)

            ! call calc_matrix_log(C_Cauchy_t, log_C_Cauchy_t, ndim)
            ! call calc_matrix_log(C_Cauchy_tm1, log_C_Cauchy_tm1, ndim)
            ! call calc_matrix_log(dC_Cauchy, dlog_C_Cauchy, ndim)

            ! eps_log_t = 0.5d0 * log_C_Cauchy_t
            ! eps_log_tm1 = 0.5d0 * log_C_Cauchy_tm1
            ! deps_log = 0.5d0 * dlog_C_Cauchy
            ! deps_log = eps_log_t - eps_log_tm1
            ...
            ! ! OLD
            ! call calc_matrix_log(B_Cauchy_t, log_B_Cauchy_t, ndim)
            ! call calc_matrix_log(B_Cauchy_tm1, log_B_Cauchy_tm1, ndim)
            ! call calc_matrix_log(dB_Cauchy, dlog_B_Cauchy, ndim)

            ! eps_log_t = 0.5d0 * log_B_Cauchy_t
            ! eps_log_tm1 = 0.5d0 * log_B_Cauchy_tm1
            ! deps_log = 0.5d0 * dlog_B_Cauchy
            ! ! deps_log = eps_log_t - eps_log_tm1


            ! ! Compute the incremental left stretch tensor (V_delta)
            ! call calc_matrix_sqrt(dB_Cauchy, dV_stretch, ndim)
            ! call calc_matrix_sqrt(B_Cauchy_t, V_stretch_t, ndim)
            ! call calc_matrix_sqrt(B_Cauchy_tm1, V_stretch_tm1, ndim)

            ! ! Compute inverse of incremental left stretch tensor
            ! call calc_matrix_inv(dV_stretch, dV_stretch_inv, ndim)
            ! call calc_matrix_inv(V_stretch_t, V_stretch_inv_t, ndim)
            ! call calc_matrix_inv(V_stretch_tm1, V_stretch_inv_tm1, ndim)

            ! ! Compute incremental rotation tensor 
            ! dR_left_rot = matmul(dV_stretch_inv, dF_grad_bar)

            ! ! Compute the logarithmic strain increment (exactly as Abaqus does)
            ! ! call calc_matrix_log(dV_stretch, log_dV_stretch, ndim)
            ! call calc_matrix_log(V_stretch_t, log_V_stretch_t, ndim)
            ! call calc_matrix_log(V_stretch_tm1, log_V_stretch_tm1, ndim)
            
            ! ! Step 1: Compute eps_log_t_rotated = R_d * eps_log_t * R_d^T
            ! do i = 1, ndim
            !     do j = 1, ndim
            !         eps_log_t_rotated(i,j) = 0.0d0
            !         do k = 1, ndim
            !             do l = 1, ndim
            !                 eps_log_t_rotated(i,j) = eps_log_t_rotated(i,j) + &
            !                     dR_left_rot(i,k) * eps_log_t(k,l) * dR_left_rot(j,l)
            !             end do
            !         end do
            !     end do
            ! end do

            ! ! Step 2: Compute deps_log = eps_log_t_rotated - eps_log_tm1
            ! do i = 1, ndim
            !     do j = 1, ndim
            !         deps_log(i,j) = eps_log_t_rotated(i,j) - eps_log_tm1(i,j)
            !     end do
            ! end do

            ! ! Step 1: Compute log strain at t and t-1 from B = F F^T
            ! call calc_matrix_log(B_Cauchy_t, log_B_Cauchy_t, ndim)
            ! call calc_matrix_log(B_Cauchy_tm1, log_B_Cauchy_tm1, ndim)

            ! do i = 1, ndim
            !     do j = 1, ndim
            !         eps_log_t(i,j)   = 0.5d0 * log_B_Cauchy_t(i,j)
            !         eps_log_tm1(i,j) = 0.5d0 * log_B_Cauchy_tm1(i,j)
            !     end do
            ! end do

            ! ! Step 2: Compute dR = polar rotation between F_t and F_tm1
            ! call calc_matrix_inv(F_grad_bar_tm1, F_grad_bar_inv_tm1, ndim)
            ! dF_grad_bar = matmul(F_grad_bar_t, F_grad_bar_inv_tm1)
            ...
            ! ! Step 3: Rotate eps_log_t to previous frame
            ! do i = 1, ndim
            !     do j = 1, ndim
            !         eps_log_t_rotated(i,j) = 0.0d0
            !         do k = 1, ndim
            ...
            ! ! Step 4: Compute Jaumann-style incremental log strain
            ! do i = 1, ndim
            !     do j = 1, ndim
            !         deps_log(i,j) = eps_log_t_rotated(i,j) - eps_log_tm1(i,j)
            !     end do
            ...
            ! deps_log = log_dV_stretch
            ! eps_log_t = log_V_stretch_t
            ! eps_log_tm1 = log_V_stretch_tm1

            stran_tm1(1) = eps_log_tm1(1,1)
            stran_tm1(2) = eps_log_tm1(2,2)
            stran_tm1(3) = eps_log_tm1(3,3)
            stran_tm1(4) = eps_log_tm1(1,2) * 2.0d0
            stran_tm1(5) = eps_log_tm1(1,3) * 2.0d0
            stran_tm1(6) = eps_log_tm1(2,3) * 2.0d0
            ...
            stran_t(1) = eps_log_t(1,1)
            stran_t(2) = eps_log_t(2,2)
            stran_t(3) = eps_log_t(3,3)
            stran_t(4) = eps_log_t(1,2) * 2.0d0
            stran_t(5) = eps_log_t(1,3) * 2.0d0
            stran_t(6) = eps_log_t(2,3) * 2.0d0
